...
{
	bool is_performance_model = false, is_kernel_map = false, is_data_movement = false;
	int id;
	Performance_model_t *performance_model;
	Kernel_map_t *kernel_map;
	Data_movement_t *data_movement;
}edge_properties_t;
...
typedef struct task_graph_node_properties
{
	Kernel_t* kernel;
	metadata_t metadata;
}task_graph_node_properties_t;

typedef boost::adjacency_list<boost::vecS, boost::vecS, boost::bidirectionalS, task_graph_node_properties_t> task_graph_t;
typedef boost::graph_traits<task_graph_t>::vertex_descriptor task_graph_vertex_descriptor_t;
typedef task_graph_t::edge_descriptor task_graph_edge_descriptor_t;

vertex_descriptor_t* get_node_array(graph_t &g)
{
	int num_nodes = boost::num_vertices(g);
	auto node_array = new vertex_descriptor_t[num_nodes];
	std::pair<vertex_iter, vertex_iter> vp;
	for (vp = boost::vertices(g); vp.first != vp.second; ++vp.first)
	{
		if (g[*vp.first].is_hardware)
			node_array[g[*vp.first].hardware->id] = *vp.first;
		else if (g[*vp.first].is_kernel)
			node_array[g[*vp.first].kernel->id] = *vp.first;
		else if (g[*vp.first].is_step)
			node_array[g[*vp.first].step->id] = *vp.first;
	}
	return node_array;
}
...
void save_graph(graph_t &g, std::string filename)
{
	...
	for (boost::tie(ei, ei_end) = boost::edges(g); ei != ei_end; ++ei)
		...
		else if (g[*ei].is_data_movement)
			g[*ei].data_movement->print(out);
	out.close();
	return;
}
...
int main()
{
	...
	// link hardwares with data_movement
	std::pair<edge_descriptror_t, bool> eh0h1 = boost::add_edge(hardware0, hardware1, KB);
	Data_movement_t cpu_to_gpu(16000000000, 1000000);
	cpu_to_gpu.type = "cpu_to_gpu";
	cpu_to_gpu.id = 16;
	KB[eh0h1.first].is_data_movement = true;
	KB[eh0h1.first].id = cpu_to_gpu.id;
	KB[eh0h1.first].data_movement = &cpu_to_gpu;
	...
	// get node sorted by their id
	vertex_descriptor_t* node_array = get_node_array(KB);
	...
	std::ifstream task_graph_node_in("task_graph_node", std::ios::in);
	int num_nodes;
	task_graph_node_in >> num_nodes;
	auto task_graph_vertices = new task_graph_vertex_descriptor_t[num_nodes];
	for (int i = 0; i < num_nodes; i++)
	{
		int id, kernel_id;
		task_graph_node_in >> id >> kernel_id;
		task_graph_vertices[id] = boost::add_vertex(task_graph);
		task_graph[task_graph_vertices[id]].kernel = KB[node_array[kernel_id]].kernel;
	}

	std::ifstream task_graph_edge_in("task_graph_edge", std::ios::in);
	int num_edges;
	task_graph_edge_in >> num_edges;
	for (int i = 0; i < num_edges; i++)
	{
		int source, dest;
		task_graph_edge_in >> source >> dest;
		boost::add_edge(task_graph_vertices[source], task_graph_vertices[dest], task_graph);
	}

	delete[] node_array;
	delete[] task_graph_vertices;
	return 0;
}